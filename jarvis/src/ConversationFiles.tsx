...
export function ConversationFiles({ sessionId, ensureSession, refreshKey, compact = false, onBusyChange, onAnalyzeTranscript }: Props) {
  ...
  return <section className={`conversation-files ${compact ? 'compact' : ''}`} aria-label="Conversation files">
    ...
    {artifacts.length > 0 && <div className="artifact-list" aria-label="Generated files">{artifacts.map(file => <article className="artifact-card" key={file.artifact_id}><span><strong>{file.display_name}</strong><small>Version {file.version} · {file.status ?? 'generated'}</small></span><div className="focus-controls"><button type="button" onClick={() => void artifactAction(file, 'open')}>Open</button><button type="button" className="quiet" onClick={() => void artifactAction(file, 'reveal')}>Reveal</button><button type="button" className="quiet" onClick={() => void artifactAction(file, 'save-as')}>Save As</button></div></article>)}</div>}
