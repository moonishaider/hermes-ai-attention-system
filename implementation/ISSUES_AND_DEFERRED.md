...
- Microphone permission, native microphone-to-spoken-reply, local wake detection, shorter natural voice projection, and immediate post-rebuild Stop speaking all pass. The project merge intends typed automatic TTS to remain off. Spoken barge-in remains available during active voice conversation, with the permanent Attention button and Control+Shift+S as discoverable fallbacks. The supported end-of-speech window is now 5.5 seconds after a first-use three-second premature-submit report; owner-visible retest remains. Local Whisper can still mishear short utterances (observed “great” as “grade”).
- A 6 August live configuration check found `voice.auto_tts: true` even though the idempotent project merge sets it to false. This may be Hermes Desktop persisting a user/session voice toggle rather than an implementation regression. Do not silently force it yet; retest one typed turn and one microphone turn, then decide the intended persistent contract.
