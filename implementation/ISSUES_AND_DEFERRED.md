...
- Microphone permission, the live microphone-to-spoken-reply loop, and automatic TTS are complete. The first speaker-only barge-in captured the interjection but did not stop speech immediately because Hermes restarted interrupted `afplay` through `ffplay`. A project-local process-only guard and regression test are complete; live barge-in retest plus visible overlay mute/cancel remain.
