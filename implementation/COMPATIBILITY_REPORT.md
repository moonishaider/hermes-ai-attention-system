...
| GitHub MCP | Official remote MCP and tool selection are current; the remote route is `https://api.githubcopilot.com/mcp`. | Two disabled logical OAuth entries use explicit read allowlists and separate owner boundaries. Live OAuth/tool discovery and provider-side read-only proof remain required. |
...
| Slack MCP | Official hosted MCP is GA at `https://mcp.slack.com/mcp`; it requires an internal/directory app and workspace approval, does not support dynamic client registration, and can expose writes. | Separate Inside Success/Mitchell entries are prepared disabled with read allowlists. App identity, actual tool names, scopes, and admin approval must be verified during OAuth. |
