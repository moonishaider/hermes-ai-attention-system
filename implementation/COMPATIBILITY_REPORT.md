...
| Google Workspace MCP | Official Gmail, Drive, and Calendar MCPs remain Developer Preview and expose mixed read/write inventories. A Google Cloud project/client is required. Hermes v0.19.1 `mcp login` can return after unauthenticated discovery without producing a token, and the pinned MCP SDK otherwise replaces the configured scope with every scope advertised by the protected resource. | A project-local OAuth probe forces a harmless read call, while the trusted plugin installs an immutable per-resource scope guard. Work Gmail/Drive/Calendar are live with exact read-only tokens and local tool allowlists; personal Google remains separate and disabled. |
