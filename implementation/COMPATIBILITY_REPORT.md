...
| Google Workspace MCP | Official Gmail, Drive, and Calendar MCPs remain Developer Preview and expose mixed read/write inventories. Program documentation requires a Google Workspace account and registered Cloud project. The personal consumer account authorizes exact scopes but provider calls return permission errors; the standard Google APIs accept the same grants. Tested access tokens omit refresh tokens. | Keep work on guarded Workspace MCP. Disable unsupported personal MCP endpoints and use three project-local, bounded, host-locked GET-only standard-API tools. Maintain per-resource freshness warnings and exact-scope reauthorization. |
