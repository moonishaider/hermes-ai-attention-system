...
## Prompt 7 claims were not carried forward automatically

- “42/42” is historical and does not prove the current installed app.
- Persistent conversations, inferred context, dependable speech finalization, direct routing, and the full Prompt 8 surfaces were re-proven against the installed product rather than inherited from Prompt 7. The final 48-item ledger records the resulting installed, visible, and automated evidence.
- The dense Prompt 7 prototype was replaced by the restrained Today, Chat, Inbox, Projects, and Actions owner path, with build/automation controls kept secondary.
...
- Acceptance completed: one normal-Chat Personal Calendar create followed by exact Undo, plus one clearly labeled unsent Personal Gmail draft. No email was sent and no work account was mutated.

## Resolved inspection limitation

An early Codex Computer Use capture attempt failed with a macOS ScreenCaptureKit stream error. Later installed-app inspection succeeded and supplied the visible evidence recorded in the final ledger, including the cadence, Focus, Radar, navigation, and recovery surfaces.
...
- Final meeting lifecycle: the owner-only strict-valid 3 August Zoom acceptance response was imported through the new bounded idempotent importer. It accepted only `zoom_readonly` sources in Inside Success, stored raw provider references only as hashes unless they were HTTPS Zoom URLs, printed no private content, and linked three confirmed evidence items to one immutable meeting ledger entry. Installed Today displayed that evidence; one reviewed local follow-up became an evidenced Inbox Meeting Task and a snapshot in `Inside Success Meeting Follow-ups`. The pre-import database backup is `backups/prompt8-pre-zoom-lifecycle-20260825T0247.sqlite3`, SHA-256 `d1229e4661ea28bcec231ec81bb6fb0850894adf1c93fec15e506754ac995a6c`, with `quick_check=ok`. No external system changed.
