...
| OPS-001–010 | Partial operational | Resumable onboarding, daily health/overlay launcher, backups, restore drill, 40 tests, and 167.3 MiB acceptance peak; human permission/account gates remain |
...
Current evidence is summarized in `implementation/PROMPT_04_ACCEPTANCE_REPORT.md`: 40 passing tests, safety/config/secret checks, bounded real-data cases, representative model evaluation, classification calibration, public-web acceptance, live microphone-to-spoken-reply acceptance, a tested macOS barge-in compatibility guard awaiting live retest, backup/restore, and resource accounting. Google, Zoom, ChatGPT export, screen, voice barge-in/visible overlay controls, and exact Slack destination/action approval remain explicit gates.
