...
| Voice and overlay | Native voice accepted; immediate speaker barge-in awaiting corrected retest | A deliberate microphone phrase passed local faster-whisper transcription, DeepSeek Flash response, and audible Edge TTS. Syed selected the British male `en-GB-RyanNeural` voice. Enabling the previously false `voice.auto_tts` fixed silent ordinary replies. The first speaker-only barge-in captured and handled the interjection but, per Syed's direct observation, did not stop speech immediately: Hermes terminated `afplay` and then incorrectly restarted audio through `ffplay`. A process-local trusted-plugin guard now prevents that fallback and passes regression tests; live retest remains. Visible overlay mute/cancel also remain. |
