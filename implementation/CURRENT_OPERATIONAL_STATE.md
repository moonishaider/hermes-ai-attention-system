...
| Voice and overlay | Native voice and speaker-only barge-in acceptance-tested; overlay controls partially accepted | A deliberate microphone phrase passed local faster-whisper transcription, DeepSeek Flash response, and audible Edge TTS. Syed selected the British male `en-GB-RyanNeural` voice. Direct synthesis measured 2.04 s first-call and 1.43 s repeat latency. Enabling the previously false `voice.auto_tts` setting fixed silent ordinary replies; a MacBook-speaker test then logged playback interruption, transcribed the interjection, and produced a shorter follow-up. Visible overlay mute/cancel still need one supervised pass. |
