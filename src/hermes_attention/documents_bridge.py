...
def attachment(record):
    ...
    return {'attachment_id':record['id'],'display_name':record['display_name'],'mime_type':record['mime'],'size_bytes':record['bytes'],'status':record['extraction_status'],'retention':record['retention'],'retention_state':record['retention_state'],'warning':'\n'.join([*record.get('warnings',[]),*([record['ocr_warning']] if record.get('ocr_warning') else [])])[:4000] or None,'ocr_status':record.get('ocr_status'),'preview':preview,'preview_truncated':sum(len(u['text']) for u in units)>4000,'sha256':record['sha256'],'citations':[{'id':u['citation'],'locator':u['locator'],'text':u['text'][:1000]} for u in units[:100]],'citation_count':len(units),'version':record['version'],'parent_id':record['parent_id'],'artifact_root_id':record.get('artifact_root_id',record['id']),'source_ids':record.get('source_ids',[]),'review_status':record.get('review_status','not_reviewed'),'conversation_id':record['conversation_id'],'turn_id':record['turn_id'],'source':record['source'],'extraction_complete':record.get('extraction_complete',False)}
