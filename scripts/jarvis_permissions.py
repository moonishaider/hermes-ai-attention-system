...
class PermissionsBridge:
    ...
    def _inventory(self,gid):
        ...
        if profile['id']=='public':targets=[{'target':{},'label':'Cookie-free public research','windowLabel':'No signed-in browser','fingerprint':{}}]
            ...
            for window in windows:
                ...
                try:
                    state=_payload(native.state(**target))
                    ...
                    for tab in state.get('tabs',[]):
                        ...
                        targets.append({'target':target,'fingerprint':{'url':tab['url'],'title':tab.get('title','')},'label':str(tab.get('title') or tab['url'])[:200],'windowLabel':str(window.get('title') or 'Browser window')[:200]})
    ...
    def _fresh_target(self,value,profile,sid,confirmed):
        ...
        native=self.native_factory(session_id=sid);target=value['target'];state=_payload(native.state(**target))
        if state.get('exact_binding') is not True:raise PermissionError('exact browser binding unavailable')
    ...
    def selection(self,operation,request):
        ...
        if operation=='browser_targets':return self._inventory(request['grantId'])
        if operation=='prepare-selection':
            ...
            with self.store.connection:
                changed=self.store.connection.execute("UPDATE browser_native_selections SET state='prepared',session_id=?,nonce=?,expires_at=? WHERE token=? AND state='inventory'",(sid,nonce,time.time()+120,row['token'])).rowcount
        ...
        if operation=='bind-selection':
            ...
            result=self.management('bind-turn',{'stage_session_id':stage,'grant_id':value['grantId'],'profile_id':profile['id'],'target':target,'owner_confirmed_target':profile['id']!='public'})
    ...
    def management(self,operation,request=None):
        ...
        if operation=='revoke':self.permissions.revoke(request['grant_id']);return {'revoked':True}
        if operation=='stop':self.permissions.stop(request['capability'],request.get('stopped',True));return {'stopped':request.get('stopped',True)}
        if operation=='bind-turn':
            ...
            if profile_id!='public':
                ...
                native=self.native_factory(session_id=sid);ObservedBrowserProbe(native,profile,owner_confirmed=request.get('owner_confirmed_target') is True).identity(target)
            existing=self.store.connection.execute('SELECT * FROM browser_turn_bindings WHERE stage_session_id=?',(sid,)).fetchone()
            if existing and (existing['revoked'] or existing['grant_id']!=grant['grant_id'] or existing['target_json']!=json.dumps(target,sort_keys=True)):raise PermissionError('API stage cannot be rebound or reactivated')
    ...
    def dispatch(self,operation,payload=None):
        ...
        target=json.loads(binding['target_json']);native=self.native_factory(session_id=origin)
